...
namespace Foundry
{
		...
		public RectangleF Bounds
		{
			get
			{
				...
				return new RectangleF(
					...
					Node.SocketBodyBounds.Y + Offset - HalfExtents,
					HalfExtents * 2,
					HalfExtents * 2
					);
			}
	...
	public class BaseNodeControl
	{
		...
		public PointF Location
		{
			get
			{
				return Node.ControlBodyBounds.Location + new SizeF(Padding, Offset);
			}
		}
		public RectangleF Bounds
        {
			get
            {
				return new RectangleF(
					Location.X,
					Location.Y,
					Node.ControlBodyBounds.Width - (Padding * 2),
					Height);
            }
        }
    ...
    public class BaseNode
	{
		...
		public BaseNode()
		{
			...
			BodyBrush = new SolidBrush(Color.FromArgb(50, 50, 50));
			TextBrush = new SolidBrush(Color.White);

			ConnectionPen = new Pen(Color.White, 7);
			ConnectionBackingPen = new Pen(Color.FromArgb(35, 35, 35), 10);

			BorderDefaultPen = new Pen(ConnectionBackingPen.Color, 3);
			BorderSelectedPen = new Pen(Color.White, 3);

			Location = new PointF(0, 0);

			_Sockets = new List<BaseNodeSocket>();
			_Controls = new List<BaseNodeControl>();
		}
		...
		public PointF Location
        {
			...
			set
            {
				if (_Location != value)
                {
					NodeMoved?.Invoke(this, new MovedArgs()
					{
		...
		public IReadOnlyList<BaseNodeSocket> Sockets
		{
			get
			{
				return _Sockets;
			...
			set
			{
				_Sockets = (List<BaseNodeSocket>)value;

				Dictionary<BaseNodeSocketDirection, float> heights = new Dictionary<BaseNodeSocketDirection, float>();
				foreach (BaseNodeSocket socket in _Sockets)
				{
					if (!heights.ContainsKey(socket.Direction)) heights.Add(socket.Direction, BaseNodeSocket.HalfExtents * 3);
					socket.Offset = heights[socket.Direction];
					heights[socket.Direction] += BaseNodeSocket.HalfExtents * 6;
				}

				SocketBodyHeight = heights.Values.Max() - BaseNodeSocket.HalfExtents * 3;
			}
		}
		private List<BaseNodeSocket> _Sockets;

		public IReadOnlyList<BaseNodeControl> Controls
		{
			get
			{
				return _Controls;
			...
			set
			{
				_Controls = (List<BaseNodeControl>)value;

				float yOffs = 15;
				foreach (BaseNodeControl control in _Controls)
				{
					control.Offset = yOffs;
					yOffs += control.Bounds.Height + 15;
				}
				ControlBodyHeight = yOffs;
			}
		}
		private List<BaseNodeControl> _Controls;

		//TODO: make private.
		public float Width { get; private set; } = 500;
		public float HeaderHeight { get; private set; } = 100;
		public float SocketBodyHeight { get; private set; } = 100;
		public float ControlBodyHeight { get; private set; } = 0;
		...
		public RectangleF Bounds
		{
			get
			{
				return new RectangleF(
					...
					Location.Y,
					Width,
					HeaderHeight + SocketBodyHeight + ControlBodyHeight);
			}
		}
		...
		public RectangleF SocketBodyBounds
		{
			get
			{
				return new RectangleF(
		...
		public RectangleF ControlBodyBounds
		{
			get
			{
				return new RectangleF(
					SocketBodyBounds.Location.X,
					SocketBodyBounds.Location.Y + SocketBodyHeight,
					Width,
					ControlBodyHeight
					);
			}
		...
		public virtual void DrawBase(Graphics g, DrawFlags flags)
		{
			...
			g.FillRectangle(BodyBrush, Rectangle.Round(SocketBodyBounds));
			g.FillRectangle(BodyBrush, Rectangle.Round(ControlBodyBounds));
	...
	public class NodeView : BaseView
	{
		...
		private float zoomMax = .55f, zoomMin = .001f;
		...
		private void OnDragL()
		{
			...
			if (mouseState == CurrentMouseState.DraggingNodes)
			{
				...
				{
					n.MoveBy(
						GetMouseState().deltaX * (1 / currentViewZoom),
						GetMouseState().deltaY * (1 / currentViewZoom)
						);
					//TrySetEdited();
				}
			}
