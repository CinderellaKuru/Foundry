...
using CryptographyTransformType = KSoft.Security.Cryptography.CryptographyTransformType;
...
namespace KSoft.Phoenix.Resource
{
        ...
        : IDisposable
    {
            ...
            TextWriter verboseOutput = null)
        {
            ...
            using (FileStream er_fs = File.OpenRead(input_file))
            using (EndianReader er = new IO.EndianReader(er_fs, Shell.EndianFormat.Big))
            using (FileStream ew_fs = new FileStream(output_file, FileMode.Create, FileAccess.Write))
            using (EndianWriter ew = new EndianWriter(ew_fs, Shell.EndianFormat.Big))
            {
        ...
        public static void CryptStream(EndianReader input, EndianWriter output, CryptographyTransformType transformType)
        {
            ...
            Security.Cryptography.PhxTEA tea = new Security.Cryptography.PhxTEA(input, output);
