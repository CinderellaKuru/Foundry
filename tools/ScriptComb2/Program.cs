...
class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        YAXSerializer ser = new YAXSerializer(typeof(Dictionary<int, Dictionary<int, ScriptLogicPrototype>>));
        ...
        //effects
        List<ScriptLogicPrototype> effectItems = new List<ScriptLogicPrototype>();
        ...
        Dictionary<int, Dictionary<int, ScriptLogicPrototype>> effects = new Dictionary<int, Dictionary<int, ScriptLogicPrototype>>();
        foreach(ScriptLogicPrototype item in effectItems)
        {
            if(effectDBIDs.ContainsKey(item.Name))
            {
                ...
                if(!effects.ContainsKey(item.DBID))
                    effects.Add(item.DBID, new Dictionary<int, ScriptLogicPrototype>());
        ...
        //conditions
        List<ScriptLogicPrototype> conditionItems = new List<ScriptLogicPrototype>();
        ...
        Dictionary<int, Dictionary<int, ScriptLogicPrototype>> conditions = new Dictionary<int, Dictionary<int, ScriptLogicPrototype>>();
        foreach (ScriptLogicPrototype item in conditionItems)
        {
            if (conditionDBIDs.ContainsKey(item.Name))
            {
                ...
                if (!conditions.ContainsKey(item.DBID))
                    conditions.Add(item.DBID, new Dictionary<int, ScriptLogicPrototype>());
    ...
    public static List<ScriptLogicPrototype> GetItems(string file, string prefix)
    {
        List<ScriptLogicPrototype> items = new List<ScriptLogicPrototype>();
        ...
        while (file.IndexOf(prefix, offset) != -1)
        {
            ...
            if (match.Success)
            {
                ScriptLogicPrototype item = new ScriptLogicPrototype()
                { 
                    Name = name,
                    Version = version,
                };
                ...
                foreach (string e in enums)
                {
                    ...
                    try
                    {
                        item.Params.Add(new ScriptLogicPrototype.Param()
                        {
                            Name = pname,
                            ID = pid,
                            Optional = poptional,
